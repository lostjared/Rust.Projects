...
impl Screen for Screen1 {
    fn draw(&mut self, can: &mut sdl2::render::Canvas<sdl2::video::Window>, w: u32, h: u32) {
        ...
        can.fill_rect(Some(Rect::new(0, 0, w, h))).expect("on drawing of rect");
    }
...
impl Screen for Screen2 {
    fn draw(&mut self, can: &mut sdl2::render::Canvas<sdl2::video::Window>, w: u32, h: u32) {
        can.set_draw_color(sdl2::pixels::Color::RGB(255, 0, 0));
        can.fill_rect(Some(Rect::new(0, 0, w, h))).expect("on drawing of rect");
    }
    fn event(&mut self, e: &sdl2::event::Event) -> Option<usize> {
        match e {
...
fn main() -> std::io::Result<()> {
    ...
    let window = video.window("Screen", width, height).resizable().opengl().build().unwrap();
...
//    let mut texture = tc.create_texture_streaming(PixelFormatEnum::RGB24, width, height).map_err(|e| e.to_string()).expect("Error on texture create");
    ...
    scr_obj.push_screen(Box::new(Screen2{}));
